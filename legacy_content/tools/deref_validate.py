"""Dereference local $ref and string path entries (restricted to legacy_content/scheets/)
then validate the fully-dereferenced page JSON against the page schema.

Usage:
    python deref_validate.py --page legacy_content/pages/scheets.page.json

Security: refs are resolved only under the `legacy_content/scheets` directory; path traversal is blocked.
"""
...
SCHEETS_DIR = (ROOT / "legacy_content" / "scheets").resolve()
SCHEMA_PATH = (ROOT / "legacy_content" / "schema" / "page_schema.json").resolve()
...
def resolve_ref_path(ref: str, base_dir: Path) -> Path:
    # Accept absolute-like starting with /legacy_content/... or relative paths
    if ref.startswith('/'):
...
def deref(node: Any, base_dir: Path, seen_files: Set[Path]) -> Any:
    ...
    if isinstance(node, str):
        if node.startswith('/legacy_content/scheets/') or node.endswith('.scheet.json'):
...
def validate_page(page_path: Path, schema_path: Path, write_deref: Path | None = None) -> int:
    ...
    scheet_schema_path = (ROOT / 'legacy_content' / 'schema' / 'scheet_schema.json')
...
def main(argv=None):
    ...
    p.add_argument('--page', default=str(ROOT / 'legacy_content' / 'pages' / 'scheets.page.json'))
