#!/usr/bin/env python3
"""Export legacy_content/codes_registry.json -> per-group json/ + markdown files.

Placed in `legacy_content/codes/tools` so maintainers can run it from the repo root:
...
# Compute repo root (three levels up from legacy_content/codes/tools)
...
def write_md(data: dict):
    ...
    for g, entries in sorted(groups.items()):
        ...
        with group_md_path.open("w", encoding="utf-8") as f:
            ...
            f.write("This file contains code snippets exported from legacy_content/codes_registry.json.\n")
            f.write("Edit this file and run `legacy_content/codes/tools/update_registry_from_md.py` to push changes back.\n\n")
