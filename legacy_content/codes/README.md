...
- Generates `docs/codes_registry.md` from `legacy_content/codes_registry.json`.

scripts/update_registry_from_md.py
- Reads `docs/codes_registry.md` and updates `legacy_content/codes_registry.json` for any code blocks preceded by markers of the form `<!-- REGISTRY_PATH: key.languages.lang -->`.
...
```bash
python legacy_content/codes/tools/export_registry_to_md.py
```

If you need to produce a custom layout for a particular group (for example `lcci`), see `legacy_content/codes/tools/generate_lcci_md_sections.py` which creates a `Section` / `Topic` structure from `legacy_content/codes/json/lcci.json`.
