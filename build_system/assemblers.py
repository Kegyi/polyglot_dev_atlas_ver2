...
def assemble_code_comparison(payload, langs, snippet_dir):
    ...
    for lang in langs:
        ...
        if code:
            ...
            warn_svg = ('<svg class="icon-warning" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" '
                        'fill="none" stroke="currentColor" stroke-width="1.6">'
                        '<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>'
                        '<path d="M12 9v4" stroke-linecap="round"/><circle cx="12" cy="17" r="1"/></svg>')
            html += f'<div class="code-card" data-lang="{lang}">'
            html += f'<div class="snippet-missing">{warn_svg}Snippet not found for {lang.upper()}</div></div>'
    html += '</div>'
